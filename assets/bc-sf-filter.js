...
BCSfFilter.prototype.buildProductGridItem = function(data, index, totalProduct) {
  ...
  if (onSale) {
    ...
    itemPriceHtml += '<span class="old-price"> ' + this.formatMoney(data.compare_at_price_min) + '</span>';
...
BCSfFilter.prototype.buildProductListItem = function(data, index, totalProduct) {
  ...
  if (onSale) {
    ...
    if (priceVaries) {
      itemPriceHtml += '<span class="money"><em>' + bcSfFilterConfig.label.from_price + ' </em>' + this.formatMoney(data.price_min) + '</span>';
    } else {
          ...
          if (colorlist.indexOf(color) == -1) {
            if (totalVariants < 4) {
              ...
              if (variant.image !== null) {
                ...
                backgroundImage = bcSfFilterConfig.general.file_url2.replace('bc-sf-filter.js', _file + '.png')
              }
